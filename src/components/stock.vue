...
<el-tabs type="border-card" class="el1" > 
  <el-tab-pane v-for="(item,index) in list" :key="item.id" :label="item.title" :offset="index > 0 ? 2 : 0">
    <div>
    <img :src="imgsrc+(index+1)+'.jpg'" class="image1">
   <div class="p2"> <h2>{{item.title}}</h2>
    <p class="p1">相关介绍：{{item.content}}</p></div>
    </div></el-tab-pane>
...
export default {
  ...
  methods: {
    async getarticlelist(){
      ...
      async test(){
      const res =await this.$http.get('/dingyang/get-articles',{params:{
        title:'', //指定查询文章的类型(模糊查询)
       type:'', //指定查询文章的类型
       pagenum:'13',  //指定查询结果的分页的页数
